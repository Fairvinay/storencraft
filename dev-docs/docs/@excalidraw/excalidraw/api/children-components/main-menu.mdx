...
By default StoreNCraft will render the `MainMenu` with default options. If you want to customise the `MainMenu`, you can pass the `MainMenu` component with the list options.
...
function App() {
  return (
    <div style={{ height: "500px" }}>
      <StoreNCraft>
        <MainMenu>
          <MainMenu.Group title="StoreNCraft items">
            <MainMenu.DefaultItems.Socials />
            <MainMenu.DefaultItems.Export />
          </MainMenu.Group>
