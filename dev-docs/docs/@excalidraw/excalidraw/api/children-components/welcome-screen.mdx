...
When the canvas is empty, StoreNCraft can show a welcome _splash_ screen with a logo, a few quick action items, and hints explaining what some of the UI buttons do. Once the user picks a tool, or has created an element on the canvas, the welcome screen will disappear.
...
<img
  ...
  alt="StoreNCraft logo: Sketch hand-drawn like diagrams."
/>
...
Use the `<WelcomeScreen.Center.Logo/>` to render a logo. By default it renders the StoreNCraft logo and name. Supply `children` to customize.
