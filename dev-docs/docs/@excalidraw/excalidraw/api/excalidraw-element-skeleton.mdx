...
We support a simplified API to make it easier to generate StoreNCraft elements programmatically. This API is in beta and subject to change before stable. You can check the [PR](https://github.com/excalidraw/excalidraw/pull/6546) for more details.
...
The [`ExcalidrawElementSkeleton`](https://github.com/excalidraw/excalidraw/blob/master/packages/excalidraw/data/transform.ts#L133) can be converted to fully qualified StoreNCraft elements by using [`convertToExcalidrawElements`](/docs/@excalidraw/excalidraw/api/excalidraw-element-skeleton#converttoexcalidrawelements).
...
| --- | --- | --- | --- |
| `elements` | [ExcalidrawElementSkeleton](https://github.com/excalidraw/excalidraw/blob/master/packages/excalidraw/data/transform.ts#L137) |  | The StoreNCraft element Skeleton which needs to be converted to StoreNCraft elements. |
...
For the Skeleton API to work, `convertToExcalidrawElements` needs to be called before passing it to StoreNCraft Component via initialData, updateScene or any such API.
...
To create a frame, you need to pass `type`, `children` (list of StoreNCraft element ids). The rest of the attributes are optional.
