...
This prop can be used to customise UI of StoreNCraft. Currently we support customising [`canvasActions`](#canvasactions), [`dockedSidebarBreakpoint`](#dockedsidebarbreakpoint) [`welcomeScreen`](#welcmescreen) and [`tools`](#tools).
...
function App() {
  ...
  return (
    <div style={{ height: "500px" }}>
      <StoreNCraft UIOptions={UIOptions} />
    </div>
  );
}
```
...
function App() {
  return (
    <div style={{ height: "500px" }}>
      <StoreNCraft UIOptions={{dockedSidebarBreakpoint: 200}}/>
    </div>
  );
}
```
