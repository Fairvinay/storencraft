...
This hook can be used to check the type of device which is being used. It can only be used inside the `children` of `StoreNCraft` component.
...
List of supported language codes. You can pass any of these to `StoreNCraft`'s [`langCode` prop](/docs/@excalidraw/excalidraw/api/props/#langcode).

#### useI18n

A hook that returns the current language code and translation helper function. You can use this to translate strings in the components you render as children of `<StoreNCraft>`.
