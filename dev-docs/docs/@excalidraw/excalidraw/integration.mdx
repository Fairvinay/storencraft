...
Throughout the documentation we use live, editable StoreNCraft examples like the one shown below.
...
function App() {
  return (
    <>
      <h1 style={{ textAlign: "center" }}>StoreNCraft Example</h1>
...
Since StoreNCraft doesn't support `server side rendering` so it should be rendered only on `client`. The way to achieve this in next.js is using `next.js dynamic import`.

If you want to only import `StoreNCraft` component you can do :point_down:
...
const StoreNCraft = dynamic(
  async () => (await import("@excalidraw/excalidraw")).StoreNCraft,
  {
    ssr: false,
  },
);
...
However the above component only works for named component exports. If you want to import some util / constant or something else apart from StoreNCraft, then this approach will not work. Instead you can write a wrapper over StoreNCraft and import the wrapper dynamically.
...
<Tabs>
  <TabItem value="StoreNCraft Wrapper" label="StoreNCraft Wrapper" >

  ```jsx showLineNumbers
  "use client";
  import { StoreNCraft, convertToExcalidrawElements } from "@excalidraw/excalidraw";
...
To use it StoreNCraft in a browser directly, use the following setup :point_down:
...
<html>
  <head>
    <title>StoreNCraft in browser</title>
    <meta charset="UTF-8" />
    <link
      rel="stylesheet"
  ...
  <body>
    <div class="container">
      <h1>StoreNCraft Embed Example</h1>
      <div id="app"></div>
    </div>
...
console.log("StoreNCraft library", ExcalidrawLib);
...
const App = () => {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(ExcalidrawLib.StoreNCraft),
    ),
  );
};
