...
This API receives the mermaid syntax as the input, and resolves to skeleton StoreNCraft elements. You will need to call `convertToExcalidraw` API to convert them to fully qualified elements that you can render in StoreNCraft.
...
try {
  ...
  // Render elements and files on StoreNCraft
} catch (e) {
...
Currently only [flowcharts](https://mermaid.js.org/syntax/flowchart.html) are supported. Oother diagram types will be rendered as an image in StoreNCraft.

### Flowchart

#### StoreNCraft Regular Shapes

**Rectangles**, **Circle**, **Diamond**, and **Arrows** are fully supported in StoreNCraft
...
Subgraphs are grouped diagrams hence they are also supported in StoreNCraft
...
**Subroutine**, **Cylindrical**, **Asymmetric**, **Hexagon**, **Parallelogram**, **Trapezoid** are not supported in StoreNCraft hence they fallback to the closest shape **Rectangle**
...
```
The above shapes are not supported in StoreNCraft hence they fallback to Rectangle
...
The [FontAwesome](https://mermaid.js.org/syntax/flowchart.html#basic-support-for-fontawesome) icons aren't support so they won't be rendered in StoreNCraft
...
Currently only [flowcharts](https://mermaid.js.org/syntax/flowchart.html) are supported. All other diagram types will be rendered as an image in StoreNCraft.
