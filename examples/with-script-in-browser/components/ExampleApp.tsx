...
export default function ExampleApp({
  ...
  const renderExcalidraw = (children: React.ReactNode) => {
    const StoreNCraft: any = Children.toArray(children).find(
      (child) =>
        React.isValidElement(child) &&
        typeof child.type !== "string" &&
        //@ts-ignore
        child.type.displayName === "StoreNCraft",
    );
    if (!StoreNCraft) {
      return;
    }
  ...
  const loadSceneOrLibrary = async () => {
    const file = await fileOpen({ description: "StoreNCraft or library file" });
  ...
  const renderMenu = () => {
    return (
      <MainMenu>
        ...
        />
        <MainMenu.Group title="StoreNCraft links">
