...
- StoreNCraft issue #9045 flowcharts: align attributes of new node [#9047](https://github.com/excalidraw/excalidraw/pull/9047)
...
- Docker build of StoreNCraft app [#7430](https://github.com/excalidraw/excalidraw/pull/7430)
...
- Support `excalidrawAPI` prop for accessing the StoreNCraft API [#7251](https://github.com/excalidraw/excalidraw/pull/7251).
...
- Expose `useI18n()` hook return an object containing `t()` i18n helper and current `langCode`. You can use this in components you render as `<StoreNCraft>` children to render any of our i18n locale strings. [#6224](https://github.com/excalidraw/excalidraw/pull/6224)
...
- Division by zero in findFocusPointForEllipse leads to infinite loop in wrapText freezing StoreNCraft [#6377](https://github.com/excalidraw/excalidraw/pull/6377)
...
- Expose component API for the StoreNCraft main menu [#6034](https://github.com/excalidraw/excalidraw/pull/6034), You can read more about its usage [here](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#MainMenu)

- Support customization for the StoreNCraft [main menu](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#MainMenu) [#6034](https://github.com/excalidraw/excalidraw/pull/6034).
...
- Any top-level children passed to the `<StoreNCraft/>` component that do not belong to one of the officially supported StoreNCraft children components are now rendered directly inside the StoreNCraft container (previously, they weren't rendered at all) [#6096](https://github.com/excalidraw/excalidraw/pull/6096).
...
- Merged `appState.currentItemStrokeSharpness` and `appState.currentItemLinearStrokeSharpness` into `appState.currentItemRoundness`. Renamed `changeSharpness` action to `changeRoundness`. StoreNCraft element's `strokeSharpness` was changed to `roundness`. Check the PR for types and more details [#5553](https://github.com/excalidraw/excalidraw/pull/5553).
...
- Added support for storing [`customData`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#storing-custom-data-on-excalidraw-elements) on StoreNCraft elements [#5592].
...
- Add display name for StoreNCraft component so it doesn't show as anonymous [#5464](https://github.com/excalidraw/excalidraw/pull/5464)
...
- Prevent browser zoom inside StoreNCraft [#5426](https://github.com/excalidraw/excalidraw/pull/5426)
...
- Expose [`serializeAsJSON`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#serializeAsJSON) helper that we use when saving StoreNCraft scene to a file [#3538](https://github.com/excalidraw/excalidraw/pull/3538).
...
- [`scrollToContent`](https://github.com/excalidraw/excalidraw/blob/master/src/components/App.tsx#L265) API now supports passing just a single StoreNCraft element, or none at all (which will default to current elements on the scene) [#3482](https://github.com/excalidraw/excalidraw/pull/3482).
...
- Make history local to a given StoreNCraft instance. This fixes a case where history was getting shared when you have multiple StoreNCraft components on the same page [#3481](https://github.com/excalidraw/excalidraw/pull/3481).
- Use active StoreNCraft component when editing text. This fixes a case where text editing was not working when you have multiple StoreNCraft components on the same page [#3478](https://github.com/excalidraw/excalidraw/pull/3478).

- Fix library being pasted off-center [#3462](https://github.com/excalidraw/excalidraw/pull/3462).

- When switching theme, apply it only to the active StoreNCraft component. This fixes a case where the theme was getting applied to the first StoreNCraft component if you had multiple StoreNCraft components on the same page [#3446](https://github.com/excalidraw/excalidraw/pull/3446).
...
- Calculate `width/height` of canvas based on excalidraw component (".excalidraw" selector) & also resize and update offsets whenever the dimensions of excalidraw component gets updated [#3379](https://github.com/excalidraw/excalidraw/pull/3379). You also don't need to add a resize handler anymore for excalidraw as its handled now in excalidraw itself.
  #### BREAKING CHANGE
  - `width/height` props have been removed. Instead now it takes `100%` of `width` and `height` of the container so you need to make sure the container in which you are rendering StoreNCraft has non zero dimensions (It should have non zero width and height so StoreNCraft can match the dimensions of containing block)
...
- Support passing a CSRF token when importing libraries to prevent prompting before installation. The token is passed from [https://libraries.storencraft.netlify.app](https://libraries.storencraft.netlify.app/) using the `token` URL key [#3329](https://github.com/excalidraw/excalidraw/pull/3329).
- #### BREAKING CHANGE
...
- Add `name` prop to indicate the name of the drawing which will be used when exporting the drawing. When supplied, the value takes precedence over `intialData.appState.name`, the `name` will be fully controlled by host app and the users won't be able to edit from within StoreNCraft [#3273](https://github.com/excalidraw/excalidraw/pull/3273).
- Export API `setCanvasOffsets` via `ref` to set the offsets for StoreNCraft[#3265](https://github.com/excalidraw/excalidraw/pull/3265).
  #### BREAKING CHANGE
...
- Add a `theme` prop to indicate StoreNCraft's theme. [#3228](https://github.com/excalidraw/excalidraw/pull/3228). When this prop is passed, the theme is fully controlled by host app.
...
- Don't overflow text beyond width of StoreNCraft [#3215](https://github.com/excalidraw/excalidraw/pull/3215).
...
- Support StoreNCraft inside scrollable container [#3018](https://github.com/excalidraw/excalidraw/pull/3018)
...
- Add view mode in StoreNCraft [#2840](https://github.com/excalidraw/excalidraw/pull/2840)
...
- Add safe check for process so StoreNCraft can be loaded via script [#2824](https://github.com/excalidraw/excalidraw/pull/2824)
...
- Exported few [Extra API's](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#extra-apis) which can be used by the host to communicate with StoreNCraft.

- Remove language picker, and add `langCode`, `renderFooter` [#2644](https://github.com/excalidraw/excalidraw/pull/2644):
  - BREAKING: removed the language picker from UI. It is now the host app's responsibility to implement a language picker if desirable, using the newly added [`renderFooter`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#renderFooter) prop. The reasoning is that the i18n should be controlled by the app itself, not by the nested StoreNCraft component.
...
- Change title to StoreNCraft after a timeout
