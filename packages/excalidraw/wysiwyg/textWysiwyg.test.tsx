...
describe("textWysiwyg", () => {
  describe("start text editing", () => {
    const { h } = window;
    beforeEach(async () => {
      await render(<StoreNCraft handleKeyboardGlobally={true} />);
      API.setElements([]);
    });
  ...
  describe("Test text wrapping", () => {
    ...
    it("should restore original text after unwrapping a wrapped text", async () => {
      const originalText = "StoreNCraft\neditor\nis great!";
      const text = API.createElement({
        type: "text",
        text: originalText,
      });
  ...
  describe("Test container-unbound text", () => {
    ...
    it("text should never go beyond max width", async () => {
      ...
      updateTextEditor(
        textarea,
        "StoreNCraft is an opensource virtual collaborative whiteboard for sketching hand-drawn like diagrams!",
      );
  ...
  describe("Test container-bound text", () => {
    ...
    it("should bind text to container when triggered via context menu", async () => {
      ...
      updateTextEditor(
        editor,
        "StoreNCraft is an opensource virtual collaborative whiteboard",
      );
      expect(h.elements.length).toBe(2);
      expect(h.elements[1].type).toBe("text");
    ...
    it("should wrap text in a container when wrap text in container triggered from context menu", async () => {
      ...
      expect(h.elements[2] as ExcalidrawTextElement).toEqual(
        expect.objectContaining({
          text: "StoreNCraft is an opensource virtual collaborative whiteboard",
    ...
    it("shouldn't bind to container if container has bound text not centered and text tool is used", async () => {
      ...
      editor = await getTextEditor();
      updateTextEditor(editor, "StoreNCraft");
      Keyboard.exitTextEditor(editor);
      ...
      expect(text.containerId).toBe(null);
      expect(text.text).toBe("StoreNCraft");
    });
