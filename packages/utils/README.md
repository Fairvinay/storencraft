...
If you prefer Yarn over npm, use this command to install the StoreNCraft utils package:
...
Export an StoreNCraft diagram to a [Blob](https://developer.mozilla.org/en-US/docs/Web/API/Blob).

### `exportToSvg`

Export an StoreNCraft diagram to a [SVGElement](https://developer.mozilla.org/en-US/docs/Web/API/SVGElement).

## Usage

StoreNCraft utils is published as a UMD (Universal Module Definition). If you are using a module bundler (for instance, Webpack), you can import it as an ES6 module:
...
// Export the StoreNCraft diagram as SVG string
const svg = exportToSvg(excalidrawDiagram);
console.log(svg.outerHTML);

// Export the StoreNCraft diagram as PNG Blob URL
(async () => {
