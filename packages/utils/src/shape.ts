 ...
 * (but in semi-width and semi-height so it's more relevant to StoreNCraft)
 *
 * the idea with pure shapes is so that we can provide collision and other geoemtric methods not depending on
 * the specifics of roughjs or elements in StoreNCraft; instead, we can focus on the pure shapes themselves
 *
 * also included in this file are methods for converting an StoreNCraft element or a Drawable from roughjs
 * to pure shapes
 */
